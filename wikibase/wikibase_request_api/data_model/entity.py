...
from wikibase_request_api.utils.data_types import class_to_data_type, data_type_to_class
from wikibase_request_api.utils.exceptions import EditError, NotFoundError, SearchError
...
class Entity(Value):
    ...
    def _create(self, content):
        ...
        self.label = self.py_wb.Label().unmarshal(
            self.entity_id, entity["labels"])
    ...
    def get(self, entity_id=None):
        ...
        try:
            ...
            raise SearchError(
                f"Could not get {self.entity_type}: {e}") from None
        ...
        self.aliases = self.py_wb.Aliases().unmarshal(
            self.entity_id, entity["aliases"])
        self.claims = self.py_wb.Claims().unmarshal(
            self.entity_id, entity["claims"])
...
class Property(Entity):
    ...
    def getPropertyId(self, prop_label):
        params = {
            "action": "wbsearchentities",
            "language": "en",
            "format": "json",
    ...
    def existProperty(self, prop_label):
...
def check_item_param(prop, param_name="item"):
    if not isinstance(prop, Item):
        raise ValueError(
            f"{param_name} parameter must be instance of Item class")


def check_prop_param(prop, param_name="property"):
    if not isinstance(prop, Property):
        raise ValueError(
            f"{param_name} parameter must be instance of Property class")
